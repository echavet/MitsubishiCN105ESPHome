...
namespace esphome {
    namespace cn105 {
        ...
        * The total size of a frame is made up of several elements:
        ...
        void CN105Climate::forward_hvac_response_to_mim(const uint8_t* buffer, size_t length) {
            if (this->mqtt_mim_handler_ != nullptr) {
                std::vector<uint8_t> response_data(buffer, buffer + length);
                this->mqtt_mim_handler_->on_hvac_response_received(response_data);
            }
        }


        void CN105Climate::processDataPacket() {
            ESP_LOGV(TAG, "Traitement d'un paquet de données...");
            this->data = &this->storedInputData[5];
            this->hpPacketDebug(this->storedInputData, this->bytesRead + 1, "READ");

            if (this->checkSum()) { // La fonction checkSum() est dans votre code
                this->lastResponseMs = CUSTOM_MILLIS;
                this->forward_hvac_response_to_mim(this->storedInputData, this->bytesRead + 1);
                processCommand(); // Traite la réponse pour mettre à jour l'état

                // --- ENVOI OPPORTUNISTE DE LA COMMANDE HA ---
                // Après avoir reçu et traité une réponse valide, le bus est libre.
                // Si une commande HA est en attente, c'est le moment parfait pour l'envoyer.
                if (this->ha_command_pending_) {
                    ESP_LOGI(TAG, "Bus libre. Envoi de la commande HA en attente...");
                    this->ha_command_pending_ = false; // On retire la commande de la file d'attente

                    // On appelle directement la logique d'envoi.
                    // Le mutex assure que l'envoi est atomique.
#ifdef USE_ESP32
