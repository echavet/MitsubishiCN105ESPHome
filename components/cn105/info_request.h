...
namespace esphome {
    ...
    struct InfoRequest {
        ...
        ) : id(id), description(description), code(code), maxFailures(maxFailures), failures(0), disabled(false), awaiting(false), soft_timeout_ms(soft_timeout_ms), interval_ms(interval_ms), last_request_time(0), canSend(nullptr), onResponse(nullptr) {
            char buf[32];
