...
namespace esphome {
    namespace cn105 {
        ...
        public:
            ...
            void set_mqtt_mim_handler(cn105::CN105MqttMim* handler);

            void send_raw_hvac_command(const std::vector<uint8_t>& data);

            void set_passive_mode_duration(uint32_t duration_ms) { this->passive_mode_duration_ms_ = duration_ms; }

            /**
             * @brief Active le mode passif, appelé lors de la réception d'une commande externe.
             */
            void activate_passive_mode();

            /**
             * @brief Vérifie si le composant est actuellement en mode passif.
             */
            bool is_in_passive_mode() const { return this->passive_mode_active_; }
        ...
        private:
            ...
            cn105::CN105MqttMim* mqtt_mim_handler_{ nullptr };

            void forward_hvac_response_to_mim(const uint8_t* buffer, size_t length);
            ...
            bool passive_mode_active_{ false };
            uint32_t passive_mode_start_ms_{ 0 };
            uint32_t passive_mode_duration_ms_{ 10000 }; // 10s par défaut
