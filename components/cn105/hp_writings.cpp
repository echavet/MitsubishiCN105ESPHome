...
namespace esphome {
    namespace cn105 {
        ...
        void CN105Climate::send_raw_hvac_command(const std::vector<uint8_t>& data) {

            this->hpPacketDebug(const_cast<uint8_t*>(data.data()), data.size(), "WRITE (MiM)");

            this->write_array(data.data(), data.size());
            ESP_LOGD("cn105", "Trame MiM envoyée à la clim (taille: %d)", data.size());
        }

        void CN105Climate::set_mqtt_mim_handler(cn105::CN105MqttMim* handler) {
            this->mqtt_mim_handler_ = handler;
        }
        ...
        */
        void CN105Climate::sendWantedSettings() {
            // Si une commande arrive de HA alors qu'on est en mode passif...
            if (this->is_in_passive_mode()) {
                ESP_LOGI(TAG, "Mode passif actif. Commande HA mise en file d'attente.");
                // ...on la met simplement en attente. Elle sera envoyée plus tard.
                this->ha_command_pending_ = true;
                return;
            }

            // Si on n'est PAS en mode passif, on exécute la logique d'envoi normale.
            if (this->isHeatpumpConnectionActive() && this->isUARTConnected_) {
                if (CUSTOM_MILLIS - this->lastSend > 300) {
                    ...
                    ESP_LOGD(TAG, "Report de l'envoi de la commande (trop récent).");
                }
            } else {
